...
export async function POST(req: NextRequest) {
  const { allowed, retryAfterSeconds } = rateLimit(`pay:investment-blueprint:${getClientIp(req)}`, 10, 10 * 60 * 1000);
  ...
  const tx_ref = `ib_${order.id}_${Date.now()}`;
  ...
  const res = await fetch("https://api.flutterwave.com/v3/payments", {
    ...
    body: JSON.stringify({
      ...
      amount: PRICE,
      currency: CURRENCY,
      redirect_url: `${SITE_URL}/investment-blueprint/verify?order_id=${order.id}`,
