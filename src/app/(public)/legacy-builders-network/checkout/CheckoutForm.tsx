...
export default function CheckoutForm({ currency, price }: Props) {
  ...
  async function applyCoupon() {
    ...
    try {
      const res = await fetch("/api/products/legacy-builders-network/coupon", {
        ...
        body: JSON.stringify({ code: coupon.trim().toUpperCase(), currency }),
      });
        ...
        <form onSubmit={handlePay} className="bg-white rounded-b-2xl px-5 sm:px-8 py-6 sm:py-8 shadow-lg space-y-5">
          ...
          <div className="text-center pb-2">
            <p className="text-4xl font-extrabold text-green-600">
              {symbol}{finalPrice.toLocaleString()} <span className="text-3xl">{currency}</span>
            </p>
            {discount > 0 && (
