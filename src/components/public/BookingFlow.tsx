...
export default function BookingFlow({ session, defaultCurrency = "NGN" }: { session: BookingSession; defaultCurrency?: string }) {
  ...
  const InfoPanel = () => (
    <div className="lg:w-72 shrink-0 space-y-6">
      <div>
        <h1 className="text-xl font-bold text-gray-900 dark:text-white leading-snug">{session.title}</h1>
        {session.description && <p className="mt-2 text-sm text-gray-500 dark:text-white/40 leading-relaxed">{session.description}</p>}
      </div>
      ...
      {selectedDate && selectedSlot && step === "form" && (
        <div className="rounded-xl bg-gray-50 dark:bg-white/3 border border-gray-100 dark:border-white/5 px-4 py-3 text-sm">
          ...
          <p className="text-gray-500 dark:text-white/40">{selectedSlot.start_time}</p>
        </div>
      )}
    </div>
  );
    ...
    <div className="flex flex-col lg:flex-row gap-8 lg:gap-16">
      ...
      <form onSubmit={handleSubmit} className="flex-1 space-y-4 max-w-sm">
        <div className="flex items-center gap-2 mb-1">
          <button type="button" onClick={() => setStep("calendar")} className="text-sm text-gray-400 dark:text-white/30 hover:opacity-70">← Back</button>
          <h2 className="text-base font-semibold text-gray-900 dark:text-white">Your details</h2>
        </div>
        <div>
        ...
        {selectedDate && (
          <div className="mt-6">
            ...
            {slotsForDate.length === 0 ? (
              <p className="text-sm text-gray-400 dark:text-white/30">No slots available for this day.</p>
            ) : (
              <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                {slotsForDate.map(slot => (
                  <button key={slot.id} type="button" onClick={() => { setSelectedSlot(slot); setStep("form"); }}
                    className="rounded-xl border border-gray-200 dark:border-white/8 px-3 py-2.5 text-sm font-medium text-gray-700 dark:text-white/60 hover:border-brand hover:text-brand dark:hover:border-brand dark:hover:text-white transition-colors">
                    {slot.start_time}
                  </button>
                ))}
              </div>
