...
export default function BlogList({
  ...
  const [featuredToast, setFeaturedToast] = useState<{ title: string; sub: string; type: "set" | "unset" } | null>(null);

  async function setFeatured(postId: string) {
    const isUnset = featuredId === postId;
    const next = isUnset ? null : postId;
    ...
    setFeaturedToast({
      type: isUnset ? "unset" : "set",
      title: isUnset ? "Removed from featured" : "Post set as featured ⭐",
      sub: post?.title ?? "",
    });
    setTimeout(() => setFeaturedToast(null), 3500);
    router.refresh();
  }
  ...
  return (
    <>
    {/* ── Featured toast ── */}
    <div className={`fixed bottom-6 right-6 z-[100] transition-all duration-300 ${featuredToast ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0 pointer-events-none"}`}>
      {featuredToast && (
        <div className={`flex items-center gap-3 rounded-2xl px-4 py-3 shadow-2xl border min-w-[280px] max-w-sm ${
          dark ? "bg-[#1c1f27] border-white/10" : "bg-white border-gray-200"
        }`}>
          <div className={`h-8 w-8 rounded-full flex items-center justify-center shrink-0 ${
            featuredToast.type === "set" ? "bg-amber-400/15 text-amber-400" : dark ? "bg-white/8 text-white/40" : "bg-gray-100 text-gray-400"
          }`}>
            <svg className="h-4 w-4" fill={featuredToast.type === "set" ? "currentColor" : "none"} stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.562.562 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
            </svg>
          </div>
          <div className="flex-1 min-w-0">
            <p className={`text-sm font-semibold ${dark ? "text-white" : "text-gray-900"}`}>{featuredToast.title}</p>
            {featuredToast.sub && (
              <p className={`text-xs mt-0.5 truncate ${dark ? "text-white/40" : "text-gray-500"}`}>{featuredToast.sub}</p>
            )}
          </div>
        </div>
      )}
